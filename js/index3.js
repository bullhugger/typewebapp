...
function drawLine() {
  let start_pos = mouseStart();
  let move_pos = mouseMove();
  canvasCtx.beginPath();
...
function mouseStart() {
  if(downFlag) {
    document.addEventListener("mousedown", drawLine, true);
  }
  return [event.clientX, event.clientY];
}

function mouseMove(moveFlag, downFlag) {
  if(downFlag == false) {
    document.removeEventListener("mousedown", mouseStart, true);
  }
  if(moveFlag == false) {
    document.addEventListener("mousemove", drawLine, true);
  }
  else {
    drawLine();
  }
